...
public class ProductionlinelistActivity extends AppCompatActivity {
    ...
    ProductionlineAdapter newsAdapter;
    ArrayList<ProductionlineArticles> articleArrayList = new ArrayList<>();
    ProductionlineViewModel newsViewModel;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_productionlinelist);

        rvHeadline = findViewById(R.id.recyclerview_productionline);

        // it is not really need for this into ProductionlinelistActivity but it hea to be sent back to ProcesscontrollermainActivity if the button is clicked
        processcontrollerFilter = (ProcesscontrollerFilter) getIntent().getSerializableExtra("processcontrollerfilter");

        // for Productionline API
        ...
        findViewById(R.id.button_processcontrollermain).setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                startActivity(new Intent(ProductionlinelistActivity.this, ProcesscontrollermainActivity.class)
    ...
    private void setupRecyclerView() {
        if (newsAdapter == null) {
            ...
            Log.e("PLU", "setupRecyclerView -> will show " + articleArrayList.size() + " tasks");

            newsAdapter = new ProductionlineAdapter(ProductionlinelistActivity.this, articleArrayList);
    ...
    private PostJSON preparePostJSON(String action, ProductionlineFilter productionlineFilter, String authKey){
        ...
        dataset = ObjectToMap(productionlineFilter);
    ...
    private void setArticleArrayList(){
        ...
        executor.execute(() -> {
            ...
            Log.e("PLU", "setArticleArrayList GetTasks.doInBackground()");
            ...
            handler.post(() -> {
                ...
                PostJSON postJSON = preparePostJSON("GETLINES"
                        , productionlineFilter
                        , ""
                ); // POST parameters for URL call

                //Log.e("PCU", "setArticleArrayList GetTasks.onPostExecute()");
                Log.e("PLU", "webaddress -> " + webaddresses.getWebaddress());

                // injects my ProcesscontrollerViewModel into newsViewModel property
                newsViewModel = new ViewModelProvider(ProductionlinelistActivity.this)
                        .get(ProductionlineViewModel.class);
                ...
                newsViewModel.getMutableLiveData().observe(ProductionlinelistActivity.this //  The LifecycleOwner which controls the observer
                        , newsResponse -> { // The observer that will receive the events. newsResponse is an object of class ProcesscontrollerResponse
                            ...
                            if (newsResponse!=null) {
                                ...
                                Log.e("PLU", "ResponseMessage ->" + newsViewModel.getProductionlineRepository().getErrMsg());
                                ...
                                Log.e("PLU", "Null response -> " + newsViewModel.getProductionlineRepository().getErrMsg());
                                Toast.makeText(  getApplicationContext()
                                        , newsViewModel.getProductionlineRepository().getErrMsg()
                                        , Toast.LENGTH_LONG
                                ).show();
                            }
                        });
